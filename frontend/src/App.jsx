...
function App() {
    ...
    const [vybranyTreningHistoriaId, setVybranyTreningHistoriaId] = useState(null);
    ...
    useEffect(() => {
        fetch('http://localhost:8080/api/treningy/dnesny-trening')
            ...
            .then(data => {
                console.log("Dnešný tréning úspešne načítaný/vytvorený:", data);
                setDnesnyTreningID(data.id);
    ...
    const sklonujCviky = (pocet) => {
        if (pocet === 1) return "cvik";
        if (pocet < 5) return "cviky";
        else return "cvikov";
    };
    ...
    const pridajCvik = () => {
        ...
        const vygenerovaneSerie = formularSerie.map(s => ({
            vaha: Number(s.vaha),
            pocetOpakovani: Number(s.pocetOpakovani)
        }));

        const novyZaznamCviku = {
            trening: {id: dnesnyTreningID},
            cvik: {
        ...
        if (!dnesnyTreningID) {
            alert("Chyba: Nepodarilo sa načítať ID dnešného tréningu! Skontroluj, či ti beží backend.");
            return;
        }
        ...
        fetch(`http://localhost:8080/api/zaznamy/trening/${dnesnyTreningID}`, {
            ...
            .then((response) => {
                if (!response.ok) throw new Error("Nepodarilo sa uložiť záznam.");
                return response.json();
            })
            .then(ulozenyZaznam => {
                console.log("Cvik úspešne pridaný pod dnešný tréning!", ulozenyZaznam);
                setTreningy(stareTreningy => [...stareTreningy, ulozenyZaznam]);
                setVlastnyCvik("");
                setVybranyCvik(PREDDEFINOVANE_CVIKY[vybranaPartia][0]);
                setFormularSerie([{id: 1, ...PRAZDNA_SERIA}]);
            })
    ...
    const pridajSeriuDoFormulara = () => {
        setFormularSerie([...formularSerie, {id: Date.now(), ...PRAZDNA_SERIA}]);
    };
    ...
    const zmenSeriuVFormulari = (id, pole, hodnota) => {
        if (hodnota === "") {
            setFormularSerie(formularSerie.map(s => s.id === id ? {...s, [pole]: ""} : s));
            return;
        }
        ...
        setFormularSerie(formularSerie.map(s => s.id === id ? {...s, [pole]: upraveneCislo} : s));
    };
    ...
    const zoskupeneTreningy = history.reduce((acc, zaznam) => {
        const trening = zaznam;
        if (!trening) return acc;

        let existujuciTrening = acc.find(t => t.id === trening.id);
        if (!existujuciTrening) {
            existujuciTrening = {
                id: trening.id,
                datum: trening.datum,
                zaznamy: []
            };
            acc.push(existujuciTrening);
        }
        existujuciTrening.zaznamy.push(zaznam);
        return acc;
    }, []);

    zoskupeneTreningy.sort((a, b) => new Date(b.datum) - new Date(a.datum));
            ...
            <aside className="sidebar">
                ...
                <nav className="sidebar-menu">
                    <button className={`menu-item ${activeTab === 'trening' ? 'active' : ''}`}
                            onClick={() => setActiveTab('trening')}>🏋️‍♂️ Dnešný tréning
                    </button>
                    <button className={`menu-item ${activeTab === 'historia' ? 'active' : ''}`}
                            onClick={() => setActiveTab('historia')}>📅 História
                    </button>
                    <button className={`menu-item ${activeTab === 'statistiky' ? 'active' : ''}`}
                            onClick={() => setActiveTab('statistiky')}>📊 Štatistiky
                    </button>
                </nav>
            </aside>
                            ...
                            <div className="add-exercise-box">
                                ...
                                <div className="form-grid">
                                    <div className="form-group row-span">
                                        ...
                                        >
                                            {Object.keys(PREDDEFINOVANE_CVIKY).map((partia) => (
                                                <option key={partia} value={partia}>{NAZVY_PARTII[partia]}</option>
                                            ))}
                                        </select>

                                        <label className="form-label label-spacing">Cvik : </label>
                                        <select value={vybranyCvik} onChange={(e) => setVybranyCvik(e.target.value)}
                                                className="form-select">
                                            {PREDDEFINOVANE_CVIKY[vybranaPartia].map((cvik) => (
                                                <option key={cvik} value={cvik}>{cvik}</option>
                                            ))}
                                            <option value="VLASTNY">-- Iný/Vlastný --</option>
                                        </select>

                                        {vybranyCvik === "VLASTNY" && (
                                            <input type="text" placeholder="Napíš názov vlastného cviku"
                                                   value={vlastnyCvik} onChange={(e) => setVlastnyCvik(e.target.value)}
                                    ...
                                    <div className="form-series-list">
                                        ...
                                        {formularSerie.map((seria, index) => (
                                            <div key={seria.id} className="form-series-row">
                                                ...
                                                <input type="number" min="1" placeholder="Opakovania"
                                                       value={seria.pocetOpakovani}
                                                       onChange={(e) => zmenSeriuVFormulari(seria.id, 'pocetOpakovani', e.target.value)}/>
                                                <input type="number" min="0" placeholder="Váha (kg)" value={seria.vaha}
                                                ...
                                                {formularSerie.length > 1 && (
                                                    <button type="button" className="btn-delete-row"
                                                            onClick={() => odstranSeriuZFormulara(seria.id)}>❌</button>
                                                )}
                                            </div>
                                        ))}
                                    </div>
                                    ...
                                    <div className="form-actions">
                                        <button type="button" className="btn-add-series"
                                                onClick={pridajSeriuDoFormulara}>➕ Pridať sériu
                                        </button>
                                        <button className="btn-submit" onClick={pridajCvik}>Pridať cvik</button>
                                    </div>
                                </div>
                            </div>
                        </div>
                                ...
                                {history.filter(workout => workout.zaznamy.length > 0).map((workout) => {
                                    ...
                                    return (
                                        <div key={workout.id} className={`history-card ${jeRozbaleny ? 'active' : ''}`}
                                             onClick={() => setVybranyTreningHistoriaId(jeRozbaleny ? null : workout.id)}>
                                            <div className="history-card-header">
                                                <h3>🏋️‍♂️ Tréning
                                                    ({workout.zaznamy.length} {sklonujCviky(workout.zaznamy.length)})</h3>
                                                <span className="workout-date">
                                                    {workout.datum ? new Date(workout.datum).toLocaleDateString('sk-SK') : "Neznámy dátum"}
                                                </span>
                                            </div>
                                            {jeRozbaleny && workout.zaznamy && (
                                                <div className="history-card-body" onClick={(e) => e.stopPropagation()}>
                                                    {workout.zaznamy.map((zaznam, index) => (
                                                        <div key={zaznam.id || index} className="history-exercise-item">
                                                            <h4>
                                                                {zaznam.cvik ? zaznam.cvik.nazovCviku : "Neznámy cvik"}
                                                                {zaznam.cvik?.svalovaPartia && <span
                                                                    className="exercise-part">({`${NAZVY_PARTII[zaznam.cvik.svalovaPartia]}`})</span>}
                                                            </h4>
                                                            <ul className="history-series-list">
                                                                {zaznam.serie && zaznam.serie.map((seria, sIndex) => (
                                                                    <li key={seria.id || sIndex}><strong>{sIndex + 1}.
                                                                        séria:</strong> {seria.pocetOpakovani}x
                                                                        @ {seria.vaha} kg</li>
                                                                ))}
                                                            </ul>
                                                        </div>
                            ...
                            <div className="sidebar-stats">
                                <div className="stats-header-container">
                                    ...
                                    {databazoveCviky.length > 0 && (
                                        <select value={vybranyCvikProgresId}
                                                onChange={(e) => setVybranyCvikProgresId(e.target.value)}
                                                className="stats-dropdown">
                                            {databazoveCviky.map(cvik => (
                                                <option key={cvik.id}
                                                        value={cvik.id}>{cvik.nazovCviku} {`(${NAZVY_PARTII[cvik.svalovaPartia]})`}</option>
                                            ))}
                                        </select>
                                    )}
                                </div>
                                    ...
                                    {progresData.length > 0 ? (
                                        <ResponsiveContainer>
                                            <LineChart data={progresData}
                                                ...
                                                <Line type="monotone" dataKey="vaha" stroke="#007bff" strokeWidth={3}
                                                      activeDot={{r: 6}}/>
                                            </LineChart>
                                        </ResponsiveContainer>
                                    ) : (
                                        <div className="no-data-message">Pre tento cvik zatiaľ nemáš zapísané žiadne
                                            tréningy.</div>
                                    )}
                                </div>
                            </div>
                        ...
                        <div className="stats-bottom-row">
                            <div className="sidebar-stats pie-chart-box">
                                ...
                                <div style={{
                                    ...
                                    <ResponsiveContainer>
                                        <PieChart>
                                            <Pie data={statistikyPartii} cx="50%" cy="50%" innerRadius={35}
                                                 outerRadius={50} paddingAngle={5} dataKey="value"/>
                                            <Tooltip contentStyle={{
                                                backgroundColor: '#222',
                                                borderColor: '#444',
                                                color: '#fff'
                            ...
                            <div className="modal-overlay" onClick={() => setIsModalOpen(false)}>
                                <div className="modal-content" onClick={(e) => e.stopPropagation()}>
                                    ...
                                    <ul className="records-list">
                                        {osobneRekordy.length === 0 ? (
                                            <li>Zatiaľ nemáš žiadne rekordy.</li>
                                        ) : (
                                            osobneRekordy.map((rekord, index) => (
                                                <li key={index}><strong>{rekord.nazovCviku}:</strong> {rekord.vaha} kg
                                                </li>
                                            ))
                                        )}
                                    </ul>
